# sum numbr of atoms of each element
#ifnd the atomic mass of each element
#multiply the number of each by their atomic mass
# add the product into the molar mass of the molicule
# divides the mass by the MM to figure number of moles in sample
# prints Molar mass
# prints number of Moles
p_table = [
...
def compute_molar_mass(elems, p_table):
    ...
    molar_mass  = 0
    for component in elems:
        element = component[0]
        quantity  = component[1]
        for list in p_table:
            if list.index(element) == element:
                mass = list[2]
        molecule_mass = mass * quantity
        molar_mass += molecule_mass
    return molar_mass
def main():
    # Get a chemical formula for a molecule from the user.
    formula = input("Input a chemical formula: ")
    # Get a mass in grams from the user.
    grams = input("What is the mass of the molecule in grams? ")

    # Call the parse_formula function to convert the
    # chemical formula given by the user to a compound
    # list that stores element symbols and the quantity
    # of atoms of each element in the molecule.
    parsed_formula = parse_formula(formula, p_table)

    molar_mass = compute_molar_mass(parsed_formula, p_table)
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    num_of_mol = molar_mass / grams
    # Compute the number of moles of the sample.
    print(f"Number of Moles: {num_of_mol}")
    print(f"Molar mass: {molar_mass}")
    # Print the molar mass.

    # Print the number of moles.
def make_periodic_table(p_table):
    for i in p_table:

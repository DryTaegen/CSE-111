def main():
    ...
    positive_answers, negative_answers = question()
    
    scoreing(positive_answers, negative_answers)
...
def question():
    ...
    positive_answers = [one, two, four, six, seven]
    negative_answers = [three, five, eight, nine, ten]
    return positive_answers, negative_answers
 
def scoreing(positive_answers, negative_answers):
    score = 0
    for letter in positive_answers:
        if letter == "A":
            score += 3
        elif letter == "a":
            score += 2
            ...
            score += 0
    for letter in negative_answers:
        if letter == "D":
            score += 3
        elif letter == "d":
            score += 2
